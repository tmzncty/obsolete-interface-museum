...
> EIA/TIA-232 family and the PC serial-port implementation：把接口家族、通信角色与一个具体主机实现分开阅读。
...
- 本期切片：V.24 **02/2000** 的 DTE/DCE 功能定义，与 IBM PC **April 1984** 手册中的 BIOS/8250 个案。
- 生命周期：接口家族的引入、主流期与衰退时间线尚未完成核验；1984 是本期 IBM 文献日期，不是 RS-232 的诞生日期。
...
3. [电气边界](electrical.md) → [物理边界](physical.md)：说明上述语义为什么还不足以决定能否接线。
...
| 层 | 当前可读内容 | 状态 / 尚缺什么 |
|---|---|---|
| Physical | [IBM 25-contact 个案与后续 PC 9-contact 语境](physical.md) | `stub`；标准/版次和实际映射交叉核验未完成 |
...
本期实际使用 **2 份 E1、1 份 E2**，见[来源账本](sources.md)；E1 并不等于“所有页面都完成验证”。没有运行仿真（E3），没有测量硬件（E4）。硬件状态为 `blocked-for-hardware`。

TIA 官方入口/合法全文、ISO 2110 相关版次全文与 IBM 9-contact 原厂资料等[原有门禁](../../research/rs232-source-map.md)仍未关闭。不得依本文或相似的 TX/RX/GND 名字直接接线，不提供 pin-to-pin、null-modem 制作或热插拔步骤。参见[硬件安全规则](../../docs/HARDWARE-SAFETY.md)。
